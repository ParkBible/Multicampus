...
class XList{
	...
	int removeLikeQueue(){
		...
		// 초기 1회 시행 : name이 10인 노드 즉 pre를 날리고 name을 꺼내길 원한다.
		post.next = pre.next;    // 날릴 노드를 가리키고 있던 post.next가 가리키는 것을 건너뛰게 한다. 현재 pre 한칸 앞 노드를 가리킴.
		pre.next = null;    // pre.next가 가리키는 대상을 지운다.
	...
	int removeLikeStack(){
		...
		while(pre.next!= null){    // pre를 제일 마지막 노드를 가리키게 땡긴다.
			post = pre;
			pre = pre.next;
		}
		...
		post.next = null;    // 제일 마지막 노드를 가리키던 포인터를 없앤다.
		tail = post;    // tail을 바꿔준다.
