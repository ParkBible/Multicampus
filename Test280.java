	...
	public static void main( String[] args ) {
		...
		*/
		...
		while(allVisited(visited) == false){
			// 방문하지 않은 spot 중에서 최소거리에 있는 spot을 리턴한다.
			int v = minDistSpot_NotVisited(dist, visited);
			System.out.println("visited : " + v);
			
			// 방문지점 0을 거쳐갔을 때의 거리가 start부터의 거리보다 짧아지면? 거리를 업뎃한다.
			// 예를 들어, 4를 방문하고 보니 3~5까지 거리가 X였는데 이게 1+2로 줄어듭니다. 그럼 거리를 업뎃
			for(int i=0; i<distance.length; i++){
